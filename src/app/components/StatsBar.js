    ...
    const fetchBookingCounts = async () => {
      try {
        ...
        const { data: totalData, error: totalError } = await supabase
          .rpc('get_booking_count_for_today', { city_name: cityName });
        ...
        const { data: completedData, error: completedError } = await supabase
          .rpc('get_completed_booking_count_for_today', { city_name: cityName });
        ...
        const { data: cancelledData, error: cancelledError } = await supabase
          .rpc('get_cancelled_booking_count_for_today', { city_name: cityName });
        ...
        const { data: sumData, error: sumError } = await supabase
          .rpc('get_total_sum_for_today', { city_name: cityName });
        ...
        const { data: commissionData, error: commissionError } = await supabase
          .rpc('get_total_commission_for_today', { city_name: cityName });
  ...
  return (
    <div className="bg-white border-t px-4 flex w-full text-sm">
      ...
      <div className="flex items-center justify-center w-24 border-r">
        ...
        <p>{totalCount !== null ? totalCount : 'Loading...'}</p>
      </div>
      <div className="flex items-center justify-center w-24 border-r">
      ...
      <div className="flex items-center justify-center border-r px-4">
        ...
        <p>{totalSum !== null ? `PHP ${totalSum.toLocaleString()}` : 'Loading...'}</p>
      </div>
      <div className="flex items-center justify-center pl-4">
        <div className="mr-4 font-semibold">Comm.</div>
        <p>{totalCommission !== null ? `PHP ${totalCommission.toLocaleString()}` : 'Loading...'}</p>
      </div>
    </div>
  );
};
